...
export class TreeNode extends Component {
    ...
    render() {
        ...
        if (unfold) {
            return (
                <div>
                    <li className={resourceType} onClick={() => this.onClick()}>
                        {nodeName}
                    </li>
                    <TreeBranch
                        resourceType={this.state.treeBranch.resourceType}
...
export class TreeBranch extends Component {
    ...
    render() {
        ...
        let missingNodes = []
        if (missing) {
            missingNodes = missing.resources.map(node => (
                <li
                    key={node.hexId}
                    className={linksTo}
                    onClick={() => actionFunc(node.hexId)}
                >
                    {node.name}
                </li>
            ))
