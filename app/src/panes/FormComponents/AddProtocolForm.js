...
export const AddProtocolForm = ({ protocol, evaluations, days, dispatch, loading }) => (
                <div className="bp3-ui-text">
                    <FormGroup label="Thème" labelFor="select-theme">
                        <HTMLSelect name="theme" onChange={(e) => dispatch(selectChanged('protocol', e))} value={protocol.theme} id="select-theme" options={selectContent.themes} fill={true}/>
                    </FormGroup>
                    <FormGroup label="Organe" labelFor="select-organ">
                        <HTMLSelect name="organ" id="organ" options={selectContent.organs[protocol.theme]} value={protocol.organ} onChange={(e) => dispatch(selectChanged('protocol', e))} fill={true}/>
                    </FormGroup>
                    <RadioGroup onChange={(e) => dispatch(radioChanged('protocol', e))} name="radio_radiochimiottt" label="Catégorie" inline selectedValue={protocol.radio_radiochimiottt}>
                    ...
                    <ControlGroup>
                        <FormGroup inline label="Protocole de chimiothérapie" labelFor="protocol-name">
                            <InputGroup name="name" onChange={(e) => dispatch(inputChanged('protocol', e))} placeholder="Nom du protocole" value={protocol.name}/>
                        </FormGroup>   
                        &nbsp;&nbsp;&nbsp;
                        <FormGroup inline label="J1 = J" labelFor="dayOneEquals" >
                            <InputGroup name="dayOneEquals" onChange={(e) => dispatch(inputChanged('protocol', e))} intent={Intent.SUCCESS} placeholder="0" id="dayOneEquals" value={protocol.dayOneEquals}/>
                        </FormGroup>
                    </ControlGroup>
