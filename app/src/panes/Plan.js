...
const Date = ({ date, dispatch }) => (
  ...
  >
    <td>
      {date.type !== "Reevaluation" && <span className="day">J{date.day}</span>}
    </td>
    <td>{date.type}</td>
    ...
    <td>
      {
        <DateInput
          ...
          onDateChange={(name, changedDate) =>
            dispatch(datesPlanDateInputChanged(name, changedDate, date.id))
          }
          ...
          <div className="right-elements">
            <FormGroup className="left-elements" label="Date de prescription">
              <DatePicker
                name="startDate"
                onChange={(date) =>
                  dispatch(planDateInputChanged("startDate", date))
                }
