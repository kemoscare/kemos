...
import { planInputChanged, planDateInputChanged, datesPlanDateInputChanged } from '../actions/plan'
import moment from 'moment'

const Date = ({ date, dispatch }) => (
        <tr className={date.type === "Reevaluation" && "evaluation"} intent={date.evaluation && Intent.SUCCESS} key={date.id}>
            <td>{date.type !== "Reevaluation" && <span className="day">J{date.day}</span>}</td>
            <td>{date.type}</td>
            ...
            <td>
                {
                    <DateInput 
                        name={'date'} 
                        onDateChange={(name, changedDate) => dispatch(datesPlanDateInputChanged(name, changedDate, date.id))} 
                        value={date.date.toDate()} 
                    />
                }
            </td>
            <td>{date.careMode === "Admission" && "Hospitalisation"}
                {date.careMode === "Home" && "À Domicile"}
                {date.careMode === "DayCare" && "Hopital de jour"}</td>
            <td>{date.type === "Reevaluation" && <DateEvaluation date={date} /> } </td>
        </tr>
)

const DateEvaluation = ({ date }) => (
        <div>
            {date.imagery && "Imagerie"}{date.imagery && <br />}
                    ...
                    <div>
                        <div className="right-elements">
                            <FormGroup className="left-elements" label="Date de prescription">
                                <DatePicker 
                                    name="startDate"
                                    onChange={(date) => dispatch(planDateInputChanged('startDate', date))} 
                            ...
                            <FormGroup label="Diagnostic" label-for="diagnostic" className="diagnosis">
                                <InputGroup 
                                    ...
                                    onChange={(event) => dispatch(planInputChanged(event))} 
                        ...
                        <div className="appointments">
                            <HTMLTable>
                                ...
                                <tbody>
                                    { plan.dates && plan.dates.map(date => <Date date={date} dispatch={dispatch} />) }
                                </tbody>
                            </HTMLTable>
                        </div>
                    </div>
